        ...
        <SheetContent side="right" class="flex w-full max-w-md flex-col gap-0 p-0">
            ...
            <SheetHeader class="border-b border-black/8 bg-[#faf9f6] px-5 py-4">
                <div class="flex items-center gap-2.5">
                    <div class="flex h-8 w-8 items-center justify-center rounded-xl bg-[#e07c281a] text-[#e07c28]">
            ...
            <div class="flex flex-col gap-2 border-b border-[#e07c2815] bg-[#e07c280a] px-4 py-3">
                <div class="flex items-center justify-between text-xs">
                    <span class="flex items-center gap-1.5 font-bold text-[#e07c28]">
                        <Truck class="h-4 w-4" />
                        {{
                    ...
                    <span class="font-mono text-[11px] font-bold text-[#4a4a57]">
                        {{ fmtRp(subtotal) }} / {{ fmtRp(freeShippingThreshold) }}
                    </span>
                </div>
            ...
            <div class="flex flex-1 flex-col gap-4 overflow-y-auto p-5">
                        ...
                        <div class="flex min-w-0 flex-1 flex-col justify-between">
                            <div>
                                <div class="flex items-start justify-between gap-2">
                                    <p class="line-clamp-2 text-xs leading-snug font-bold text-[#1c1c22]">
                                        {{ item.name }}
                                    </p>
                                    <button
                            ...
                            <div class="mt-2 flex items-end justify-between">
                                <span class="font-mono text-xs font-extrabold text-[#e07c28]">
                                    {{ fmtRp(item.price * item.qty) }}
                                </span>

                                <!-- Qty Selector -->
                                <div class="flex items-center gap-1.5 rounded-xl border border-black/8 bg-white p-1">
                                    <button
                                    ...
                                    <span class="px-1.5 font-mono text-xs font-bold text-[#1c1c22]">
                                        {{ item.qty }}
                                    </span>
                                    <button
                                        @click="emit('update-qty', item.id, 1)"
                ...
                >
                    <ShoppingCart class="mb-3 h-12 w-12 stroke-1 text-[#c8c8d5]" />
                    <p class="text-sm font-bold text-[#4a4a57]">Keranjang Anda Kosong</p>
            ...
            <!-- Footer -->
            <SheetFooter v-if="items.length > 0" class="flex-col gap-3 border-t border-black/8 bg-[#faf9f6] px-5 py-4">
                <div class="flex items-center justify-between text-xs">
                    <span class="text-[#9090a0]">Subtotal Produk</span>
                    <span class="font-mono text-base font-extrabold text-[#1c1c22]">
                        {{ fmtRp(subtotal) }}
                    </span>
                </div>
                <div class="flex items-center gap-2 text-[10px] font-medium text-[#22a15a]">
                    <ShieldCheck class="h-3.5 w-3.5 shrink-0" />
                    <span>Transaksi Terproteksi Escrow Toko Instan Guarantee</span>
                </div>
                <Button
                    variant="amber"
                    size="lg"

...
<template>
    <div
        class="group flex cursor-pointer flex-col overflow-hidden rounded-2xl shadow-md border-2 transition-all duration-300 hover:shadow-2xl hover:-translate-y-1.5 active:scale-[0.98] relative"
        :style="{ borderColor: 'var(--brand-soft)', background: 'linear-gradient(180deg, #ffffff 40%, var(--brand-soft) 100%)' }"
        @click="emit('click')"
    >
        <!-- ── 4-Hex Theme Top Gradient Bar ── -->
        <div
            class="h-1.5 w-full transition-opacity duration-300"
            :style="{ background: 'linear-gradient(90deg, var(--brand), var(--brand-secondary), var(--brand-accent), var(--brand-strong))' }"
        />
        ...
        <div class="relative aspect-square w-full overflow-hidden bg-[#f5f4f0]">
            ...
            <div
                ...
                :style="{ background: 'linear-gradient(135deg, var(--brand-strong), var(--brand-strong))' }"
            >
            ...
            >
                <Heart
                    ...
                    :class="isInWishlist(product.id) ? 'fill-[var(--brand-strong)] text-[var(--brand-strong)]' : 'text-[#c8c8d5]'"
                />
            </button>
            ...
            <div
                ...
                class="absolute bottom-0 left-0 right-0 flex items-center gap-1 bg-gradient-to-t from-black/60 to-transparent px-2 pt-3 pb-1.5"
            >
                <span
                    class="rounded-md px-1.5 py-0.5 text-[9px] font-black text-white uppercase tracking-wider shadow-xs"
                    :style="{ background: 'linear-gradient(90deg, var(--brand-accent), var(--brand-secondary))' }"
                >
                    Gratis Ongkir
                </span>
            </div>
        ...
        <div class="flex flex-1 flex-col gap-1 p-3 transition-colors duration-300">
            <!-- Product name — 2 lines max -->
            <p class="line-clamp-2 text-xs font-bold leading-snug text-[#1c1c22]">
                {{ product.name }}
            </p>
            ...
            <div class="mt-0.5">
                ...
                <p v-if="product.originalPrice" class="text-[9px] text-[#9090a0] line-through leading-none font-mono">
                    {{ product.originalPrice }}
                </p>
                <!-- Main price -->
                <p class="text-sm sm:text-base font-black leading-tight font-mono" :style="{ color: 'var(--brand-strong, #e0405a)' }">
                    {{ product.price }}
                </p>
            </div>

            <!-- Rating + Sold -->
            <div class="mt-auto flex items-center gap-1.5 pt-1.5 border-t border-[var(--brand)]/15">
                <div class="flex items-center gap-0.5">
                    <Star class="h-3 w-3 fill-amber-400 stroke-amber-400" />
                    <span class="text-[10px] font-bold text-[#4a4a57]">{{ product.rating }}</span>
                </div>
                <span class="text-[10px] text-[#c8c8d5]">·</span>
                <span class="text-[10px] font-medium text-[#9090a0]">{{ formatSold(product.sold) }} terjual</span>
            </div>

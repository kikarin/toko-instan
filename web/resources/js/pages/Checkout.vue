...
<template>
    ...
    <StorefrontLayout :cartCount="cartItems.length">
        <main class="mx-auto w-full max-w-[1400px] px-4 pt-4 pb-28 font-sans sm:p-6">
            <div class="mb-6">
                <p class="mb-1 text-xs font-extrabold tracking-widest text-brand uppercase">
                    Langkah Terakhir
                </p>
            ...
            <div v-if="isEmpty"
                ...
                <Button size="lg" class="font-bold bg-brand text-brand-foreground hover:opacity-90 border-0"
            ...
            <template v-else>
                ...
                <div v-if="isGuest && !guestMode"
                    ...
                    <div class="flex w-full max-w-xs flex-col gap-3 sm:flex-row sm:max-w-md">
                        ...
                        <Button size="lg" variant="outline" class="flex-1 font-bold" @click="goLogin">
                            Login Dulu
                        </Button>
                    </div>
                </div>

                <form v-else @submit.prevent="handleCheckoutSubmit"
                    ...
                    <div class="order-2 flex flex-col gap-5 lg:order-1 lg:col-span-7">
                            ...
                            <CardContent class="flex flex-col gap-4 p-0">
                                <template v-if="availableAddresses && availableAddresses.length > 0">
                                    <div class="flex flex-col gap-3">
                                        <div v-for="addr in availableAddresses" :key="addr.id"
                                            ...
                                            <div class="flex flex-col gap-1">
                                                <div class="flex flex-wrap items-center gap-2">
                                                    <span class="font-bold text-foreground">{{ addr.recipient_name
                                                        }}</span>
                                                    <Badge v-if="addr.is_default" variant="secondary"
                                                        class="bg-black/10 text-[10px]">Utama</Badge>
                                                    <Badge variant="outline" class="text-[10px]">{{ addr.label }}
                                                    </Badge>
                                                </div>
                                            ...
                                            <div class="mt-1 flex h-5 w-5 items-center justify-center rounded-full border-2"
                                                :class="selectedAddressId === addr.id
                                                        ? 'border-brand'
                                                        : 'border-border'
                                                    ">
                                                <div v-if="selectedAddressId === addr.id"
                                                    class="h-2.5 w-2.5 rounded-full bg-brand"></div>
                                            </div>
                                        </div>

                                        <!-- Manual Address Option -->
                                        <div @click="selectedAddressId = 'manual'"
                                            ...
                                            <div class="flex h-5 w-5 items-center justify-center rounded-full border-2"
                                                :class="selectedAddressId === 'manual'
                                                        ? 'border-brand'
                                                        : 'border-border'
                                                    ">
                                                <div v-if="selectedAddressId === 'manual'"
                                                    class="h-2.5 w-2.5 rounded-full bg-brand"></div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <!-- Manual Form -->
                                <div v-if="!availableAddresses?.length || selectedAddressId === 'manual'"
                                    class="mt-2 flex flex-col gap-4 rounded-2xl border border-border bg-muted/30 p-4">
                                    <div class="flex flex-col gap-1.5">
                                        <Label for="c-name" class="text-xs font-bold text-foreground">Nama Lengkap
                                            Penerima *</Label>
                                        <Input id="c-name" v-model="manualName" placeholder="Contoh: Budi Santoso"
                                            required class="bg-card" />
                                    </div>

                                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                        <div class="flex flex-col gap-1.5">
                                            <Label for="c-email" class="text-xs font-bold text-foreground">Email
                                                *</Label>
                                            <Input id="c-email" v-model="manualEmail" type="email"
                                                placeholder="budi@email.com" required class="bg-card" />
                                        </div>
                                        <div class="flex flex-col gap-1.5">
                                            <Label for="c-phone" class="text-xs font-bold text-foreground">Nomor
                                                WhatsApp / HP *</Label>
                                            <Input id="c-phone" v-model="manualPhone" type="tel"
                                                placeholder="08123456789" required class="bg-card" />
                                        </div>
                                    </div>

                                    <div class="flex flex-col gap-1.5">
                                        <Label for="c-province" class="text-xs font-bold text-foreground">Provinsi
                                            *</Label>
                                        <Select v-model="manualProvince" required>
                                            ...
                                            <SelectContent class="max-h-60">
                                                <SelectGroup>
                                                    <SelectItem v-for="prov in provinces" :key="prov.id"
                                                        :value="prov.name">
                                                        {{ prov.name }}
                                                    </SelectItem>
                                                </SelectGroup>
                                    ...
                                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                        <div class="flex flex-col gap-1.5">
                                            <Label for="c-city" class="text-xs font-bold text-foreground">Kota/Kabupaten
                                                *</Label>
                                            <Select :model-value="manualCity" required :disabled="!manualProvince"
                                        ...
                                        <div class="flex flex-col gap-1.5">
                                            <Label for="c-district" class="text-xs font-bold text-foreground">Kecamatan
                                                *</Label>
                                        ...
                                        <div class="flex flex-col gap-1.5">
                                            <Label for="c-postal" class="text-xs font-bold text-foreground">Kode Pos
                                                *</Label>
                                            <Input id="c-postal" v-model="manualPostalCode" required type="number"
                                                maxlength="5" placeholder="12190" class="bg-card" />
                                        </div>
                                    </div>
                                    <div class="flex flex-col gap-1.5">
                                        <Label for="c-address" class="text-xs font-bold text-foreground">Alamat Lengkap
                                            *</Label>
                                        <Textarea id="c-address" v-model="manualAddress" rows="2"
                                            placeholder="Nama Jalan, Gedung, No. Rumah, RT/RW, Patokan" required
                                ...
                                <div v-else class="mt-2 flex flex-col gap-1.5 rounded-2xl border border-border p-4">
                                    <Label for="c-email-only" class="text-xs font-bold text-foreground">Email * <span
                                            class="font-normal text-muted-foreground">(Untuk konfirmasi
                                            pesanan)</span></Label>
                                    <Input id="c-email-only" v-model="manualEmail" type="email"
                        ...
                        <Card class="p-6">
                            ...
                            <CardContent class="flex flex-col gap-2.5 p-0">
                                <p v-if="!manualCity && selectedAddressId === 'manual'"
                                    class="text-xs text-muted-foreground">
                                    Pilih kota tujuan dulu untuk menghitung ongkir.
                                </p>
                                <p v-else-if="isLoadingRates" class="text-xs text-muted-foreground">
                                    Menghitung ongkir...
                                </p>
                                <p v-else-if="couriers.length === 0" class="text-xs text-muted-foreground">
                                ...
                                <Button v-if="couriers.length === 0 && (manualCity || selectedAddressId !== 'manual')"
                                    type="button" variant="outline" class="h-9 text-xs font-bold"
                                    :disabled="isLoadingRates" @click="loadShippingRates">
                                    Coba hitung ongkir
                                </Button>
                                <div v-for="c in couriers" :key="c.id" @click="selectedCourier = String(c.id)"
                                    ...
                                    <div class="flex items-center gap-3">
                                        <div class="flex h-4 w-4 items-center justify-center rounded-full border-2"
                                            :class="String(selectedCourier) === String(c.id)
                                                    ? 'border-brand'
                                                    : 'border-border'
                                                ">
                                            <div v-if="String(selectedCourier) === String(c.id)"
                                    ...
                                    <span class="font-mono text-xs font-bold text-brand">
                                        {{ c.cost_fmt }}
                                    </span>
                                </div>
                            </CardContent>
                        ...
                        <Card class="p-6">
                            ...
                            <CardContent class="flex flex-col gap-2 p-0">
                                <div class="flex gap-2">
                                    <Input v-model="voucherCode" placeholder="KODE PROMO" class="h-10 uppercase"
                                        :disabled="!!appliedVoucher" />
                                    <Button v-if="!appliedVoucher" type="button" variant="outline" class="h-10"
                                        :disabled="isApplyingVoucher" @click="applyVoucher">
                                        Pakai
                                    </Button>
                                    <Button v-else type="button" variant="ghost" class="h-10" @click="clearVoucher">
                                        Hapus
                                    </Button>
                                </div>
                                <p v-if="appliedVoucher" class="text-xs text-brand">
                    ...
                    <div class="order-1 flex flex-col gap-5 lg:order-2 lg:col-span-5">
                        <Card class="sticky top-24 p-6">
                            <CardTitle class="mb-4 flex items-center justify-between text-base">
                                <span>Ringkasan Pesanan</span>
                                <Badge variant="default" class="text-[10px]">{{ cartItems.length }} produk</Badge>
                            </CardTitle>
                            ...
                            <div class="flex flex-col gap-2 border-b border-border py-4 text-xs">
                                ...
                                <div v-if="discount > 0" class="flex justify-between text-muted-foreground">
                                    <span>Diskon voucher</span>
                                    <span class="font-mono font-semibold">-{{ fmtRp(discount) }}</span>
                                </div>
                                <div v-if="tax > 0" class="flex justify-between text-muted-foreground">
                                    <span>PPN {{ ppnRate }}%</span>
                                    <span class="font-mono font-semibold">{{ fmtRp(tax) }}</span>
                                </div>
                                <div class="flex justify-between pt-1 font-medium text-[#22a15a]">
                            ...
                            <div class="flex items-center justify-between pt-4 pb-5">
                                <div>
                                    ...
                                    <p class="mt-0.5 font-mono text-2xl leading-none font-extrabold text-brand">
                                        {{ fmtRp(grandTotal) }}
                                    </p>
                                </div>
                            </div>

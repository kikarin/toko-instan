...
import { Palette, Star, Sparkles, ImageIcon, Save, Plus, Trash2, Home, BadgeCheck, Quote } from 'lucide-vue-next';
import { ref } from 'vue';
...
const featuredIds = ref<number[]>(props.store?.showcase?.featured_product_ids ?? []);
const testimonials = ref<Testimonial[]>(
    (props.showcase?.testimonials ?? []).map((t) => ({ ...t, rating: t.rating || 5 })),
);
...
function refreshPreviewStyle() {
    ...
    if (typeof document !== 'undefined') {
        ...
        root.style.setProperty('--brand-secondary', previewColors.value.secondary);
        ...
        root.style.setProperty('--brand-soft', hexToRgba(previewColors.value.primary, 0.15));

        root.style.setProperty('--sidebar-primary', previewColors.value.primary);
        root.style.setProperty('--sidebar-accent', hexToRgba(previewColors.value.primary, 0.15));
        root.style.setProperty('--sidebar-accent-foreground', previewColors.value.primary);
...
function hexToRgba(hex: string, alpha: number): string {
    ...
    if (clean.length !== 6) return 'rgba(224, 124, 40, 0.15)';
    ...
    <AppLayout title="Tampilan & Konten" activePage="Tampilan & Konten">
        <main class="mx-auto flex w-full max-w-4xl flex-col gap-5 p-4 sm:p-6">
            <div>
                ...
                <h1 class="flex items-center gap-2 text-2xl font-extrabold text-[#1c1c22]">
            ...
            <div class="grid gap-5 lg:grid-cols-[1fr_260px]">
                <div class="flex flex-col gap-5">
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle class="flex items-center gap-2 text-sm">
                                <Sparkles class="h-4 w-4 text-[#e07c28]" /> Pilih Tema
                            </CardTitle>
                        ...
                        <CardContent>
                            <div class="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-3">
                                ...
                                >
                                    <span class="flex gap-1.5">
                                        <span class="h-4 w-4 rounded-full" :style="{ backgroundColor: cfg.colors.primary }" />
                                        <span class="h-4 w-4 rounded-full" :style="{ backgroundColor: cfg.colors.secondary }" />
                                        <span class="h-4 w-4 rounded-full" :style="{ backgroundColor: cfg.colors.accent }" />
                                        <span class="h-4 w-4 rounded-full" :style="{ backgroundColor: cfg.colors.strong }" />
                                    </span>
                                    <span class="text-xs font-bold text-[#1c1c22]">{{ cfg.label }}</span>
                                    <span class="text-[10px] text-[#9090a0]">{{ cfg.font }}</span>
                                </button>
                                ...
                                >
                                    <span class="flex items-center gap-1.5">
                                        <Palette class="h-4 w-4 text-[#e07c28]" />
                                        <span class="text-xs font-bold text-[#1c1c22]">Custom</span>
                                    </span>
                                    <span class="text-[10px] text-[#9090a0]">Ubah warna sesuai keinginan</span>
                                </button>
                            </div>
                            ...
                            <div class="mt-4 flex flex-wrap items-center gap-x-4 gap-y-3">
                                <div v-for="(cb, ci) in colorTokens" :key="ci" class="flex items-center gap-2">
                                    <Label class="w-16 text-xs">{{ cb.label }}</Label>
                                    <input
                                        type="color"
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle class="flex items-center gap-2 text-sm">
                                <Home class="h-4 w-4 text-[#e07c28]" /> Hero Banner
                            </CardTitle>
                            <CardDescription class="text-xs">
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle class="flex items-center gap-2 text-sm">
                                <Star class="h-4 w-4 text-[#e07c28]" /> Produk Unggulan
                            </CardTitle>
                            <CardDescription class="text-xs">
                                Pilih produk yang ditampilkan di bagian unggulan.
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            <div v-if="products.length" class="grid gap-2 sm:grid-cols-2">
                                ...
                                >
                                    ...
                                    <div v-else class="flex h-9 w-9 items-center justify-center rounded-lg bg-[#f5f4f0]">
                                        <ImageIcon class="h-4 w-4 text-[#c8c8d5]" />
                                    </div>
                                    <div class="min-w-0">
                                        <p class="truncate text-xs font-bold text-[#1c1c22]">{{ p.name }}</p>
                                        <p class="text-[10px] text-[#9090a0]">{{ p.price }}</p>
                                    </div>
                                </label>
                            </div>
                            <p v-else class="text-xs text-[#9090a0]">
                                Belum ada produk aktif. Tambahkan produk dulu di halaman Produk.
                            </p>
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle class="flex items-center gap-2 text-sm">
                                <BadgeCheck class="h-4 w-4 text-[#e07c28]" /> Tentang Toko
                            </CardTitle>
                        </CardHeader>
                        <CardContent class="flex flex-col gap-3">
                            <div class="flex flex-col gap-1.5">
                    ...
                    <Card>
                        <CardHeader class="flex flex-row items-start justify-between">
                            <div>
                                <CardTitle class="flex items-center gap-2 text-sm">
                                    <Quote class="h-4 w-4 text-[#e07c28]" /> Testimoni
                                </CardTitle>
                                <CardDescription class="text-xs">Ulasan dari pelanggan Anda.</CardDescription>
                            </div>
                        ...
                        <CardContent class="flex flex-col gap-3">
                            ...
                            >
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <select v-model.number="t.rating" class="h-8 rounded-lg border border-black/10 bg-white px-2 text-xs">
                                        <option v-for="n in 5" :key="n" :value="n">{{ n }} bintang</option>
                                    </select>
                                </div>
                            </div>
                            <p v-if="!testimonials.length" class="text-xs text-[#9090a0]">
                    ...
                    <Card>
                        <CardContent class="flex items-center justify-between p-4">
                            <div>
                                <p class="text-sm font-bold text-[#1c1c22]">Tampilkan kontak</p>
                                <p class="text-xs text-[#9090a0]">Info kontak Anda dari pengaturan toko.</p>
                            </div>
                            <Switch :checked="contactShow" @update:checked="(v: boolean) => (contactShow = v)" />
                        </CardContent>
                    </Card>
                ...
                <div class="lg:sticky lg:top-24 lg:self-start">
                    <p class="mb-2 text-[10px] font-black uppercase tracking-widest text-[#9090a0]">Pratinjau</p>
                    <div
                        class="overflow-hidden rounded-2xl border border-black/8 bg-[#16131f] shadow-lg"
                        :style="previewStyle"
                    >
                        <div class="bg-gradient-to-br from-[#16131f] via-[#231e35] to-[#16131f] px-5 py-6">
                            <p class="text-[9px] font-bold uppercase tracking-widest text-white/40">Halaman Toko</p>
                            <h2 class="mt-1 text-base font-black text-white">{{ hero.title || 'Judul Hero' }}</h2>
                            <p class="mt-1 text-[11px] leading-relaxed text-white/50">
                                {{ hero.subtitle || 'Sub judul hero akan tampil di sini.' }}
                            </p>
                            <div
                        ...
                        <div class="border-t border-white/8 bg-white px-5 py-4">
                            <p class="text-xs font-black text-[#1c1c22]">{{ about.title || 'Tentang Toko' }}</p>
                            <p class="mt-1 text-[11px] text-[#9090a0]">{{ about.text || 'Deskripsi toko Anda...' }}</p>
                            <div class="mt-3 flex items-center gap-1.5">
                                <span class="h-2.5 w-2.5 rounded-full" :style="{ backgroundColor: 'var(--brand)' }" />
                                <span class="text-[10px] font-semibold text-[#4a4a57]">
